package com.example.laboratorio02diaz
...
class MainActivity : AppCompatActivity() {
    ...
    private fun addListener () {
        actionCalculate.setOnClickListener{
            ...
            val bmiToDigits = String.format("%.2f", bmi).toFloat()
    ...
    private fun calculateBMI (weight : Float, height : Float): Float {
        return weight / (convertFromCentimetersToMeters(height) * convertFromCentimetersToMeters(height))
    }
    private fun convertFromCentimetersToMeters (a : Float) : Float {
